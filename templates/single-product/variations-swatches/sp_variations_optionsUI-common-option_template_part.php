...
ob_start();
wbc()->load->template("publics/swatches/$woo_dropdown_attribute_html_data['type']", array('args'=>$woo_dropdown_attribute_html_data['args'],'term'=>$term,'type'=>$variable_item_data['options_loop_type'][$term->slug]['type']));
...
if(!in_array($variable_item_data['options_loop_type'][$term->slug]['type'],array('dropdown_image','dropdown_image_only','dropdown'))) {                 
    ...
    $template = array(
        'type' => 'header',
        'tag' => 'li',
        'class' => 'ui image middle aligned variable-item '.esc_attr( $variable_item_data['options_loop_type'][$term->slug]['type'] ).'-variable-item '.esc_attr( $variable_item_data['options_loop_type'][$term->slug]['type'] ).'-variable-item-'.esc_attr( $term->slug ) esc_attr( $variable_item_data['options_loop_selected_class'][$term->slug]['selected_class'] ),
