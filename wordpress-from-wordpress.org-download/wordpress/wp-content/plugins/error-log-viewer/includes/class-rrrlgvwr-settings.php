...
if ( ! class_exists( 'Rrrlgvwr_Settings_Tabs' ) ) {
	class Rrrlgvwr_Settings_Tabs extends Bws_Settings_Tabs {
		...
		public function save_options() {
			...
			if ( $this->options['php_error_log_visible'] == 1 ) {
			...
			foreach ( $this->wp_error_files as $key => $file ) {
				...
				if ( is_readable( $file ) ) {
					...
					if ( $file == $rrrlgvwr_php_error_path && $this->options['php_error_log_visible'] == 1 ) {
						continue;
					}
					if ( $this->options[ $subname ] == 1 ) {
			...
			$this->options['email_user']    = get_user_by( 'login', $_POST['rrrlgvwr_email_user'] ) ? sanitize_text_field( $_POST['rrrlgvwr_email_user'] ) : '';

			if ( 1 == $this->options['send_email'] && $this->options['file_path'] ) {
				if ( is_email( $_POST['rrrlgvwr_email'] ) ) {
					$this->options['email']				= $_POST['rrrlgvwr_email'];
					$this->options['frequency_send']	= absint( $_POST['rrrlgvwr_frequency_send'] );
					$this->options['hour_day']			= array_key_exists( $_POST['rrrlgvwr_hour_day'], $rrrlgvwr_periods ) ? $_POST['rrrlgvwr_hour_day'] : 3600;
			...
			} elseif ( 1 == $this->options['send_email'] && array() == $this->options['file_path'] ) {
		...
		public function tab_settings() {
			...
			if ( ! $error_logging_enabled ) {
				...
				$php_error_mes	= esc_attr( $rrrlgvwr_php_error_path );
			} ?>
            <h3 class="bws_tab_label"><?php _e( 'Error Log Viewer Settings', 'error-log-viewer' ); ?></h3>
			<?php $this->help_phrase(); ?>
            <hr>
			<div class="bws_tab_sub_label"><?php _e( 'PHP Error Log', 'error-log-viewer' ); ?></div>
                    ...
                    <tr>
						...
						<?php if ( file_exists( $rrrlgvwr_php_error_path ) ) { ?>
                            ...
                            <td>
								...
								echo ': ' . date( 'Y-m-d H:i:s', filemtime( $rrrlgvwr_php_error_path ) ); ?>
                            </td>
						<?php } ?>
                    </tr>
                </table>
			...
			<?php } ?>
			<div class="bws_tab_sub_label"><?php _e( 'WordPress Error Log', 'error-log-viewer' ); ?></div>
                ...
                <table class="form-table">
                    <?php foreach ( $this->wp_error_files as $key => $file ) {
                        ...
                        <tr>
                            <th scope="row" class="th-full">
                                <label>
                                    <input type="checkbox" name="<?php echo $subname;?>" <?php if ( isset( $this->options[ $subname ] ) ) checked( $this->options[ $subname ] ); ?> />
                                    <?php echo esc_attr( $name ); ?>
                                </label>
                            </th>
                            <td><?php echo $file; ?></td>
		...
		public function tab_notifications() {
            ...
            <h3 class="bws_tab_label"><?php _e( 'Email Notification Settings', 'error-log-viewer' ); ?></h3>
			<?php $this->help_phrase(); ?>
            <hr>
            <table class="form-table">
                <tr>
                    <th><?php _e( 'Email Notification', 'error-log-viewer' ); ?></th>
                    <td>
                        ...
                        <span class="bws_info"><?php _e( 'Enable to receive email notifications.', 'error-log-viewer' ); ?></span>
                    </td>
                </tr>
            </table>
			<table id="rrrlgvwr-notifications-table" class="form-table">
                <tr>
                    <th scope="row"><?php _e( "Send Email Notifications to", 'error-log-viewer' ); ?></th>
                    <td>
                        <fieldset>
                            <label><input class="bws_option_affect" data-affect-show="#rrrlgvwr-user-email" data-affect-hide="#rrrlgvwr-custom-email" type="radio" name="rrrlgvwr_to_email" value="user" <?php checked( 'user', $this->options['to_email'] ); ?> /> <?php _e( 'User', 'error-log-viewer' ); ?></label><br />
                            <div id="rrrlgvwr-user-email">
                                <select name="rrrlgvwr_email_user">
                                    <option disabled><?php _e( "Select a username", 'error-log-viewer' ); ?></option>
                            ...
                            <label><input class="bws_option_affect" data-affect-hide="#rrrlgvwr-user-email" data-affect-show="#rrrlgvwr-custom-email" type="radio" name="rrrlgvwr_to_email" value="custom" <?php checked( 'custom', $this->options['to_email'] ); ?> /> <?php _e( 'Custom email', 'error-log-viewer' ); ?></label><br />
                            <div id="rrrlgvwr-custom-email">
                                <input type="text" name="rrrlgvwr_email" value="<?php echo $this->options['email']; ?>" maxlength="500" /><br />
                            </div>
                        </fieldset>
                    </td>
                </tr>
				<tr>
					<th scope="row"><?php _e( 'Send Email', 'error-log-viewer' ); ?></th>
					<td>
						<fieldset>
                            <span><?php _e( 'every', 'error-log-viewer'); ?></span>
							<input type="number" class="rrrlgvwr-middle" min="1" name="rrrlgvwr_frequency_send" value="<?php echo $this->options['frequency_send']; ?>" />
							<select name="rrrlgvwr_hour_day">
                                <?php foreach ( $rrrlgvwr_periods as $seconds => $period ) { ?>
                                    <option value="<?php echo $seconds ?>" <?php selected( $seconds, $this->options['hour_day'] ); ?>><?php echo  $period ?></option>
                                <?php } ?>
							</select>
						</fieldset>
					</td>
