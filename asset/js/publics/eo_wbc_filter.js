...
function eo_wbc_filter_render_html(data,render_container) {
	...
	jQuery.each(jQuery(".products a,.product-listing a"),function(index,element) {

		var href=jQuery(element).attr('href');
		if(typeof(href)!==typeof(undefined) && typeof(href)==='string'){
			if(href.indexOf('?')==-1) {
...
/*if(eo_wbc_object.disp_regular=='1'){
	*/
	...
	//flag indicates if to show products in tabular view or woocommerce's default style.		
		...
		if(init_call) {
			jQuery(form_selector).find("[name='paged']").val('1');
			jQuery(form_selector).find("[name='last_paged']").val('1');

			jQuery(form_selector).find("[name='_category']").val(jQuery(form_selector).find("[name='_current_category']").val());
			jQuery(form_selector).find("[name='_attribute']").val("");
		}
		...
		jQuery.ajax({
			...
			beforeSend:function(xhr){
				...
				/* Code block to cancel last call to save network resource */
...
jQuery(document).ready(function($){
	...
	jQuery(".eo_wbc_srch_btn:eq(2)").click(function(){					
		...
		jQuery('[name="paged"]').val('1');
		jQuery.fn.eo_wbc_filter_change(false,'form[id*="eo_wbc_filter"]:eq(0)');

	});	

	jQuery('.orderby[name="orderby"]').on('change click',function(evt){
		evt.preventDefault(); 
		evt.stopPropagation();

		if(jQuery('form[id*="eo_wbc_filter"] [name="orderby"]').length<=0) {
			jQuery('form[id*="eo_wbc_filter"]').append('<input type="hidden" name="orderby" value="menu_order"/>');
		}

		jQuery('form[id*="eo_wbc_filter"]').find('[name="orderby"]').val(jQuery(this).val());
		jQuery.fn.eo_wbc_filter_change(false,'form[id*="eo_wbc_filter"]:eq(0)');
	});
