    ...
    things supposed to be managed by the sp_variations 
        ...
        -   random 
            --  each extensions will have their own module, so create variations.assets.php file in each 7 extensions. in some b had already created -- to a. ask b if you have questions. 
                --  also create load asset function in model -- to a. ask b if questions.  done
                --  and then always call load_asset from render_ui calling stack -- to a. done
                  -- wbc (load_asset) 
                ...
                --  and then inside variations.assets.php crete the js module, the module name should be based on the singleton function name -- to a done
                --  and then inside module create the general fucntions like init, init_private, bootstrap, preprocess, preprocess_data and so on -- to s done 
                  -- process_attribute_types (size,shape,color)
                  -- process_images (other 4 extention)
                --  and then from here mostly instead of hosting things like managing events or binding clicks etc. it will just listen to events. so implement all subscribe statements /*subscribe call in function*/-- to a 
...
window.document.splugins.wbc.variations.swatches.core = function( base_container_selector, configs ) {
    ...
    _this.base_container = jQuery( window.document.splugins.common._o( _this.configs, 'base_container_selector') /*ACTIVE_TODO_OC_START -- to d. base_container_selector ACTIVE_TODO_OC_END*/ ||  '.variations_form' );      
