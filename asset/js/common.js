...
// put the is item page and is category page conditions for below underscore js port statement -- to s 
//     --  and also put same both conditions for while during exporting the three modules namely swatches, gallery_images and sp_slzm -- to s 
//     --  and the is_category_page and is_item_page flgs are not set properly on the js.vars asset php file so set it there. look at public handler file if required -- to s 
//     --  and also put the php side is_category_page and is_item_page conditions for underscore js loading since that is going to be used on these two page only -- to s 
//     --  and also the export statement still miss one line of calling init, I think. so look at the form builder asset php and if that is the case then put init function call under is js module where it is exported under .api object -- to s 
//         --  all modules means also the pagination and filters module in the filters js. and also the module in sp tv template js file. -- to s 
...
//                             --  for now that is just needed on the gallery_images layer, so lets just use there or -- to a 
//          -   pages 
//              // -- category page 
//              // -- item page 
...
//                             --  for now set the container class hardcoded -- to a 
...
//                          --  and on this regard gallery_images module also need to create listener function for variation change event and listen to that, and when that is detected -- to h or -- to a 
//                                 --  do cover the sub tasks below also or -- to a  
...
//                                     --  to be sure here and ensure neat flow check once how that other plugin we were exploring does updating the templates, means are they erasing and then appending or -- to a 
//                                         --  it just crossed the mind that they were actually updating the html so erase and append at once, but at there must be some resolve or other smoothing function calls which might be of interest to us or -- to a 
...
//     }; 
>>>>>>> 384773f98f88056e73ddb7b022fb82cfdff99c71
...
//         below types var neet to be prepaired in preprocess_data -- to a 
//           -- also neet to clear type managment in wbc variation class. simply set type in extra perance -- to b or -- to a 
