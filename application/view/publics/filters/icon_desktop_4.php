...
<div class="<?php echo esc_attr($width_class); ?>" data-tab-group="<?php echo esc_attr($tab_set); ?>">
	<div style="display: inline-block;" class="ui three wide field icon_header">
		...
		<?php endif; ?>
		<?php if($reset): ?>
			&nbsp; <span class="ui grey text" style="cursor: pointer;" onclick="reset_icon(event,'<?php echo esc_attr($term->slug); ?>')">&nbsp;<u><?php esc_html(sp_wbc_ext_lang("reset", 'woo-bundle-choice')) ?></u></span>
		<?php endif; ?>
	</div>	
			...
			<?php foreach ($list as $filter_icon): ?>
				<div style="padding: 0px;" title="<?php esc_attr($filter_icon["name"]); ?>"
					class="eo_wbc_filter_icon column <?php echo $non_edit ? 'none_editable':'' ?> 
						<?php echo $filter_icon['mark'] ? 'eo_wbc_filter_icon_select':''?> ui image" data-single_select="<?php echo esc_attr($is_single_select); ?>"
					...
					<div style="/*height: <?php echo esc_attr($icon_width); ?>;*/display: flex;">
						<img src='<?php echo esc_url($filter_icon['mark']?$filter_icon['select_icon']:$filter_icon['icon']); ?>' class="ui mini image" style="<?php echo esc_attr($icon_css); ?>;margin: auto;display: block;" data-imgsrc="<?php echo esc_url($filter_icon['icon']); ?>" data-toggleimgsrc="<?php echo esc_url($filter_icon['select_icon']); ?>"/>
					</div>
