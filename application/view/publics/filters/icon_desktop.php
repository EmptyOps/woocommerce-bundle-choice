...
?>
	<div class="<?php echo esc_attr($width_class)/*$width_class*/; ?>" data-tab-group="<?php _e($tab_set); ?>">
		<p>
			<span class="ui header"><?php echo esc_attr(($title))/*($title)*/; ?></span>
			<?php if($help): ?>
		...
		<div class="ui tiny images ui equal width center aligned grid" style="text-align: center;" data-reset="reset_icon(new Event('click'),'<?php echo esc_attr($term->slug)/*$term->slug*/; ?>')" data-filter-slug="<?php echo esc_attr($term->slug)/*$term->slug*/; ?>">				
			<?php foreach ($list as $filter_icon): ?>
				<div title="<?php $filter_icon["name"]; ?>"
					...
					data-filter="<?php echo esc_attr($term->slug)/*$term->slug*/; ?>" style="border-bottom: 2px solid transparent;<?php echo esc_html($icon_css)/*$icon_css*/; ?>"
