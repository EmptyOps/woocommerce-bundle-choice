...
<div class="onclick_redirect step <?php echo ($step==$order)?'active ':' '; ?>" data-begin="<?php echo get_term_link(wbc()->wc->get_term_by('slug', $first_slug, 'product_cat')->term_id,'product_cat').'EO_WBC=1&BEGIN='.$first_slug.'&STEP=1'; ?>" <?php _e((!empty(\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$clickable_breadcrumb) and !empty(\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$first_url))?'data-clickable_breadcrumb="'.\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$first_url.'"':''); ?>>  
    <div class="ui equal width grid" style="width: 100%;margin-top: -1em !important;" >
        ...
        <div class="column eowbc_breadcrumb_font"><?php esc_html_e($order); ?></div>
        <div class="column eowbc_breadcrumb_font" style="text-align: left;">                       
            ...
            <?php else:?>
                ...
                if(empty($view_url) || $view_url=='#'){
                    ?>
                    <div class="description eowbc_breadcrumb_font"><?php echo esc_html(spext_lang("Choose a", 'woo-bundle-choice')); ?></div>
                    <div class="title eowbc_breadcrumb_font"><?php echo esc_html($first_name); ?></div>
                    <div>&nbsp;</div>
                    <?php
                } else {
                    ...
                    <div><u><a href="<?php echo esc_url($view_url); ?>"> <?php echo esc_html(spext_lang("View", 'woo-bundle-choice')); ?></a></u>&nbsp;|&nbsp;<u><a href="<?php echo esc_url($remove_url); ?>" data-remove-url="<?php echo esc_url($remove_url); ?>"><?php esc_html_e(wbc()->options->get_option('appearance_breadcrumb','appearance_breadcrumb_change_action_text','Change',true,true)); ?></a></u></div>
            <?php } endif; ?>                    
        </div>                
        <div class="column">
            <?php if(!empty($first_icon)): ?>
                <img src="<?php echo esc_url($first_icon); ?>" style="margin: auto;" class="ui mini image">
            <?php endif;?>
        </div>
    </div>
</div>
