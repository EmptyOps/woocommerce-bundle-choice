...
<div class="onclick_redirect step <?php echo esc_attr(($step==$order)?'active ':' ')/*($step==$order)?'active ':' '*/; ?>" data-begin="<?php echo get_term_link(wbc()->wc->get_term_by('slug', esc_attr($second_slug)/*$second_slug*/, 'product_cat')->term_id,'product_cat').'EO_WBC=1&BEGIN='.esc_attr($second_slug)/*$second_slug*/.'&STEP=1'; ?>" <?php _e((!empty(\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$clickable_breadcrumb) and !empty(\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$second_url))?'data-clickable_breadcrumb="'.\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$second_url.'"':''); ?>>            
    ...
    <div class="ui equal width grid" style="width: 100%;margin-top: -1em !important;">
        ...
        <div class="column eowbc_breadcrumb_font" style="text-align: left;">
            ...
            <?php else:?>
                ...
                <div><u><a href="<?php echo esc_attr($view_url)/*$view_url*/; ?>"> <?php spext_lang("View", 'woo-bundle-choice') ?></a></u>&nbsp;|&nbsp;<u><a href="<?php echo esc_attr($remove_url)/*$remove_url*/; ?>" data-remove-url="<?php echo esc_attr($remove_url)/*$remove_url*/; ?>"><?php _e(wbc()->options->get_option('appearance_breadcrumb','appearance_breadcrumb_change_action_text','Change',true,true)); ?></a></u></div>
            <?php } endif; ?>                    
        </div>                
        <div class="column">
            <?php if(!empty($second_icon)): ?>
                <img src="<?php echo esc_html($second_icon)/*$second_icon*/; ?>" style="margin: auto;" class="ui mini image">
            <?php endif;?>
        </div>
    </div>            
</div>
