...
<div class="onclick_redirect step <?php echo ($step==$order)?'active ':' '; ?>" data-begin="<?php echo /*get_term_link(wbc()->wc->get_term_by('slug', $second_slug, 'product_cat')->term_id,'product_cat').'EO_WBC=1&BEGIN='.$second_slug.'&STEP=1'*/wbc()->common->beautify_url_data(get_term_link(wbc()->wc->get_term_by('slug', $second_slug, 'product_cat')->term_id,'product_cat').'EO_WBC=1&BEGIN='.$second_slug.'&STEP=1'); ?>" <?php _e((!empty(\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$clickable_breadcrumb) and !empty(\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$second_url))?'data-clickable_breadcrumb="'/*.\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$second_url*/wbc()->common->beautify_url_data(.\eo\wbc\model\publics\component\EOWBC_Breadcrumb::$second_url).'"':''); ?>>            
