...
?>
    <script>
      var eo_wbc_outer_container=void 0;jQuery.send_error=0;

      jQuery(document).ready(function($){

       $(".ui.modal").find(".cancel").on("click",(function(){jQuery(".ui.modal").modal("hide")}));

        $(".ui.modal").find(".approve").on('click',function(){
          if(!jQuery.send_error){
            jQuery.send_error=1,$(this).text("Sending error report...");
            
            jQuery.post('<?php echo site_url('/wp-admin/admin-ajax.php');?>',{resolver:'eowbc_send_error_report', _wpnonce:jQuery( jQuery('input[name="eowbc_send_error_report_wpnonce"]')[0] ).val(), action: 'eowbc_ajax', saved_tab_key: 'setting_status_log',is_sent_from_front_end: 1},function(data){                   
              data?(jQuery(".ui.modal").find(".actions").html('<div class="ui large green inverted button error_sent">Ok</div>'),jQuery(".ui.modal .content").html("<h5>Thank you for sending error report, Sphere Plugins Support Team will soon get in touch with you. It generally takes 12 hours.</h5>"),jQuery.send_error=0):($(".ui.modal").find(".approve").text("Resend an error report now!"),jQuery.send_error=0);
            });                
          }
        });

       jQuery(".ui.modal").on("click",".error_sent",(function(){jQuery(".ui.modal").modal("hide")})),jQuery(".ui.modal").on("click",".view_log",(function(){return document.location.href=jQuery(this).attr("href"),jQuery(".ui.modal").modal("hide"),!1}));
      ...
      function eo_wbc_error_popup(type,msg) {
        ...
        <?php if(current_user_can('manage_options') && empty(wbc()->sanitize->get('eo_wbc_button_testing'))): ?>
          ...
          jQuery(".ui.modal").modal({onApprove:function(){return!1}}).modal("show");
        <?php endif; ?>                  
        ...
        //below testing service is not implemented yet for buttons and not implemented in general as well  
           ...
           var btn_test_service_status=0;null!=eo_wbc_outer_container&&null!=eo_wbc_outer_containers&&eo_wbc_outer_containers.length>0&&(btn_test_service_status=1==eo_wbc_outer_containers.length?1:2);
