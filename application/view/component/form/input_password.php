...
if(!empty($id) /*and !empty($label)*/){
	...
	<div class="<?php echo !empty($size_class)?esc_attr($size_class)/*$size_class*/:''; ?> field">
		...
		?>	
		<input type="password" <?php echo !empty($attr)?esc_attr($attr)/*$attr*/:''; ?> id="<?php echo esc_attr($id)/*$id*/; ?>" name="<?php echo esc_attr($id)/*$id*/; ?>" class="ui input <?php echo !empty($class)?esc_attr($class)/*$class*/:''; ?>">	
		<?php
