...
if(!empty($id) /*and !empty($label)*/){
	...
	<div class="<?php echo esc_attr(!empty($size_class)?$size_class:''); ?> field">
		...
		?>		
		<textarea spellcheck="false"  id="<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($id); ?>" class="ui input <?php echo esc_attr(!empty($class)?$class:''); ?>" placeholder="<?php echo esc_attr($placeholder); ?>" ><?php echo isset($value)?esc_html($value):''; ?></textarea>
		<?php
