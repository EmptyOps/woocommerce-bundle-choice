...
if(!empty($id) /*and !empty($label)*/){
	...
	<div class="<?php echo !empty($size_class)?esc_attr($size_class)/*$size_class*/:''; ?> field">
		...
		?>	
		<input type="number" <?php echo !empty($attr)?esc_attr($attr)/*$attr*/:''; ?> id="<?php echo esc_attr($id)/*$id*/; ?>" name="<?php echo esc_attr($id)/*$id*/; ?>" class="ui input <?php echo !empty($class)?esc_attr($class)/*$class*/:''; ?>" placeholder="<?php echo !empty($placeholder)?esc_attr($placeholder)/*$placeholder*/:''; ?>" value="<?php echo isset($value)?esc_attr($value)/*$value*/:'0'; ?>">	
		<?php
