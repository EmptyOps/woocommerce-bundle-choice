...
<?php if(empty(wbc()->sanitize->get('step')) or (!empty(wbc()->sanitize->get('step')) and wbc()->sanitize->get('step')!=3 )) { 
	...
	$wp_create_nonce_wp_create_nonce = wp_create_nonce('sample_data_jewelry');
	$apply_filters_apply_filters = apply_filters('eowbc_catattr_sample_data_resolver_path','');
	$feature_key_feature_key = _e($feature_key);
	$next_url_next_url = echo($next_url);
	$admin_url_admin_url = admin_url( 'admin-ajax.php' );
	$_category_category = sizeof($_category);
	$sample_data_obj_sample_data_obj = echo $sample_data_obj->get_model()->get_attributes_size();
	$inline_script = 
  ...
  "                  //step 2 redirect;\n" .
  "                  var data = {                  \n" .
  "                    '_wpnonce': '".$wp_create_nonce_wp_create_nonce."',\n" .
  "                    'action':'eowbc_ajax',\n" .
  "                    'resolver':'sample_data/catattr',\n" .
  "                    'resolver_path':'".$apply_filters_apply_filters."', \n" .
  "                    'feature_key':'".$feature_key_feature_key."',\n" .
  ...
  "                      window.location.href=".$next_url_next_url.";\n" .
  "                      return false;\n" .
  "                  },\n" .
  ...
  "                     //window.location.href=".$next_url_next_url.";  //commented since can't allow redirect on error etc.\n" .
  "                      return false;     \n" .
  "                  }\n" .
  "              });  \n" .
  "                  return false;\n" .
  ...
  "              }\n" .
  "\n" .
  "              jQuery.post('".$admin_url_admin_url."', data, function(response) {\n" .
  ...
  "                    main_categories_size = ".$_category_category.";\n" .
  "                    cat_value = ".$sample_data_obj_sample_data_obj.";\n" .
  "                    btn_total = cat_value;\n" .
  "                  } else if(attr_value>0){\n" .
  "                    process_flag = 'attr';\n" .
  "                    btn_label = 'Attributes';\n" .
  "                    attr_value = ".$sample_data_obj_sample_data_obj.";\n" .
  ...
  "      });\n";
	...
	$get_product_size_get_product_size = echo($sample_data_obj->get_model()->get_product_size());
	$page_page = echo(admin_url('admin.php?page=eowbc'));
	$eowbc_product_sample_eowbc_product_sample = echo apply_filters('eowbc_product_sample_data_resolver_path','');
	$inline_script = 
  "jQuery(document).ready(function($) {            \n" .
  "\n" .
  "          var eo_wbc_max_products=".$get_product_size_get_product_size.";            \n" .
  "          function eo_wbc_add_products(index){\n" .
  "\n" .
  "              if(index>=eo_wbc_max_products){\n" .
  "                  \n" .
  "                  window.location.href=".$page_page.";\n" .
  "                  return false;\n" .
  "              }\n" .
  "\n" .
  ...
  "                  '_wpnonce': '".$wp_create_nonce_wp_create_nonce."',\n" .
  "                  'action':'eowbc_ajax',\n" .
  "                  'resolver':'sample_data/".$feature_key_feature_key."',\n" .
  "                  'resolver_path':'".$eowbc_product_sample_eowbc_product_sample."', \n" .
  "                  'product_index':index \n" .
  "              };\n" .
  "\n" .
  "              jQuery.post('".$admin_url_admin_url."', data, function(response) {\n" .
