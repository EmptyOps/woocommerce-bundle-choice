...
if( empty($mode) || ( $mode != "setup_wizard" && $mode != "plain" ) ) {
	...
	add_filter( 'admin_footer_text',function($footer_text){
	    ...
	    return sprintf(eowbc_lang('If you like %1$s ').constant('EOWBC_NAME').eowbc_lang(' %2$s please leave us a %3$s  rating. A huge thanks in advance!'),"<strong>","</strong>","<a href='https://wordpress.org/support/plugin/woo-bundle-choice/reviews?rate=5#new-post' target='_blank' class='wc-rating-link' data-rated='Thanks :)'>★★★★★</a>");
	});
