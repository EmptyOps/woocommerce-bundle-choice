...
class SP_WBC_Extensions_Bootstrap {
	...
	if( wbc()->sanitize->get('is_test') == 1 ) {
		...
		wbc_pr("SP_WBC_Extensions_Bootstrap run1");	 
	}
  ...
  //   	}
		...
		if((function_exists('is_ajax') and is_ajax()) or defined('WP_AJAX')) {
			...
			if( wbc()->sanitize->get('is_test') == 1 ) {
				...
				wbc_pr("SP_WBC_Extensions_Bootstrap run2");	 
			}
			//TODO implement below hooks for extensions when necessary
			...
			if( wbc()->sanitize->get('is_test') == 1 ) {
				...
				wbc_pr("SP_WBC_Extensions_Bootstrap run3");	 
			}
			// system core
			$this->system_core();

			/*if(class_exists('SP_WBC_Http_Handler')){*/
				$Extensions_Http_Handler = new Extensions_Http_Handler( $this->SP_WBC_Extension );
	...
	private function system_core(){
		...
		//	core loaders
		//
		if( method_exists($this->SP_WBC_Extension->singleton_function()(), 'system_core_loader') ) {
			$this->SP_WBC_Extension->singleton_function()()->system_core_loader();
		}

		//	core init 
		//
		if( method_exists($this->SP_WBC_Extension->singleton_function()(), 'system_core_init') ) {
			
			$this->SP_WBC_Extension->singleton_function()()->system_core_init();
		}
	...
	public static function activate( $network_wide /*$network_wide hiren commented argument as that seems to be a problem as to why WP is not calling this function but anyway its not verified if it fixes the problem and works with all versions and standards so do the needful */ ) {			
		...
		(new Extensions_Activate( new SP_WBC_Extension( $plugin ) ))->run();
	}
	...
	public static function deactivate( $network_wide ) {
		...
		(new Extensions_Deactivate( new SP_WBC_Extension( $plugin ) ))->run();
	}
	...
	public static function uninstall( $network_wide ) {
		...
		self::safe_load();
		(new Extensions_Uninstall( new SP_WBC_Extension( $plugin ) ))->run();
	}	
