...
use eo\wbc\system\core\publics\SP_WBC_Base_Model_Publics ;

class SP_WBC_Model_Loop_Gallery_Zoom extends SP_WBC_Base_Model_Publics {
	...
	public function render_ui(){
		...
		foreach($hook_array as $hook_key) {
			add_filter($hook_key /*'sp_slzm_zoom_image_loop_js_tempalte'*/,function($html, $index, $image) use($hook_key){
				...
				// 	wbc_pr( "SP_wbc_Model_Gallery_Zoom index ".$index );
				// 	wbc_pr('SP_wbc_Model_Gallery_Zoom index key_is = '.$hook_key.' index_is = ' . $index);
				// 	wbc_pr( $html );	
				// }
	            ...
	            // if(wbc()->sanitize->get('is_test') == 1 || wbc()->sanitize->get('is_test') == 9) {
 		        ...
 		        //     wbc_pr('End SP_wbc_Model_Gallery_Zoom index key_is = '.$hook_key.' index_is = ' . $index);
				// 	wbc_pr( $html );	            	
				...
				foreach($hook_array as $hook_key) {
					...
					if(wbc()->config->product_variations_configs()['is_gallery_images_type_based_template'] == 1) {
						...
						if( wbc()->sanitize->get('is_test') == 1 ) {
							...
							wbc_pr("SP_wbc_Model_Gallery_Zoom wbc");
							wbc_pr($constant_key);
						}
