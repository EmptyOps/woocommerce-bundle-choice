...
use eo\wbc\system\core\publics\SP_WBC_Query;

class SP_WBC_Model_Query extends SP_WBC_Query {
		...
		if(($input_method != 'GET' && $input_method != 'POST' && $input_method != 'REQUEST') || !empty(wbc()->sanitize->{strtolower($input_method)}('eo_wbc_filter'))) {
		    ...
		    add_filter('pre_get_posts',function($query ) use($input_method, $additional_data) {		    		
				...
				if( wbc()->sanitize->get('is_test') == 1 ) {
					wbc()->common->var_dump( "SP_WBC_Model_Query wbc prepare_query pre_get_posts input_method = ".$input_method );
				}
		  ...
		  //   	if(apply_filters('eowbc_filter_override',false) and (!empty($_REQUEST['eo_wbc_filter']))) {
				...
				// 	if( wbc()->sanitize->get('is_test') == 1 ) {
				// 		wbc()->common->var_dump( "SP_WBC_Model_Query wbc prepare_query pre_get_posts eowbc_filter_override ".$input_method );
				// 	}
