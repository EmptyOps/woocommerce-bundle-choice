...
class EOWBC_Breadcrumb
{
    ...
    public static function eo_wbc_breadcumb_first_html($step,$order){
        ...
        } elseif($template=='theme_alt') {
            // die("2130000");
            wbc()->load->template('publics/breadcrumb/theme_alt_first_step_desktop', array("step"=>$step,"order"=>$order,"first"=>self::$first,"view_url"=>(!empty(wbc()->sanitize->get('FIRST')) ? self::eo_wbc_breadcrumb_view_url(wbc()->sanitize->get('FIRST'),$order):'#'),"change_url"=>(!empty(wbc()->sanitize->get('FIRST'))?self::eo_wbc_breadcrumb_change_url($order,wbc()->sanitize->get('FIRST')):'#'),'first_icon'=>self::$first_icon,"first_name"=>self::$first_name,'first_slug'=>self::$first_slug)); 
    ...
    public static function eo_wbc_breadcumb_second_html($step,$order){        
        ...
        if($template=='theme'){
            ...
            wbc()->load->template('publics/breadcrumb/theme_alt_second_step_desktop', array("step"=>$step,"order"=>$order,"second"=>self::$second,"view_url"=>(!empty(wbc()->sanitize->get('SECOND'))?self::eo_wbc_breadcrumb_view_url(wbc()->sanitize->get('SECOND'),$order):'#'),"change_url"=>(!empty(wbc()->sanitize->get('SECOND'))?self::eo_wbc_breadcrumb_change_url($order,wbc()->sanitize->get('SECOND')):'#'),'second_icon'=>self::$second_icon,"second_name"=>self::$second_name,'second_slug'=>self::$second_slug));
    ...
    public static function eo_wbc_breadcrumb_view_url($product_id,$order,$product = null){
        ...
        if(self::eo_wbc_breadcrumb_get_category($product_id)==self::$first_slug or (!empty($product) and wbc()->wc->is_variation_object($product) and self::eo_wbc_breadcrumb_get_category($product->get_parent_id())==self::$first_slug)  /*get_option('eo_wbc_first_slug')*/) {
           ...
           // echo "text 1";
            return (/*$product_url.wbc()->common->http_query(array('EO_WBC'=>1,'BEGIN'=>wbc()->sanitize->get('BEGIN'),'STEP'=>sanitize_text_field($order),'FIRST'=>sanitize_text_field(empty(wbc()->sanitize->get('FIRST'))?'':wbc()->sanitize->get('FIRST')),'SECOND'=>sanitize_text_field(empty(wbc()->sanitize->get('SECOND'))?'':wbc()->sanitize->get('SECOND')),'EO_VIEW'=>1))*/wbc()->common->beautify_url_data($product_url.wbc()->common->http_query(array('EO_WBC'=>1,'BEGIN'=>wbc()->sanitize->get('BEGIN'),'STEP'=>sanitize_text_field($order),'FIRST'=>sanitize_text_field(empty(wbc()->sanitize->get('FIRST'))?'':wbc()->sanitize->get('FIRST')),'SECOND'=>sanitize_text_field(empty(wbc()->sanitize->get('SECOND'))?'':wbc()->sanitize->get('SECOND')),'EO_VIEW'=>1))));
