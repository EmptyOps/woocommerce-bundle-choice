...
use \eo\wbc\SP_WBC_Extensions_Bootstrap;
use eo\wbc\model\utilities\SP_WBC_Theme_Adaption_Check;

if(!class_exists('SP_WBC_Plugin_Index_Class') ) {

	class SP_WBC_Plugin_Index_Class {
		...
		public function construct_init($childClassObj) {			
			...
			if( property_exists($this->SP_WBC_Extension->singleton_function()(), 'config') && method_exists($this->SP_WBC_Extension->singleton_function()()->config, 'explicit_class_loader_config') ) {
		...
		public function getSP_Extension() {			
			return $this->SP_WBC_Extension;
		}

		public function SP_WBC_Extension() {			
		...
		public function load_helpers() {
			...
			*/
      		...
      		$singleton_functionUpper = strtoupper( $this->SP_WBC_Extension->singleton_function() );
			...
			if(!empty($helpers)){
				...
				foreach ($helpers as $helper=>$helper_class) {					
					if( file_exists(constant($singleton_functionUpper.'_HELPERS_DIR')."helper-${helper_class}.php") ) {
						...
						$this->SP_WBC_Extension->singleton_function()()->set_helper( $helper, $helper_class::instance() );
					}
				}	
			}
		}
		...
		public function load_library() {
			...
			*/
			...
			$library = array('load'=>'WBC_Loader','migration'=>'SP_WBC_Migration','sanitize'=>'SP_WBC_Sanitize','validate'=>'SP_WBC_Validate','rest'=>'SP_WBC_REST');
		...
		public function explicit_class_loader() {
			...
			$singleton_functionUpper = strtoupper( $this->SP_WBC_Extension->singleton_function() );
			$explicit_class_loader_config = $this->SP_WBC_Extension->singleton_function()()->config->explicit_class_loader_config( $singleton_functionUpper );
		...
		public function define_constants() {
			...
			defined( $singleton_functionUpper.'_DIRECTORY') || define($singleton_functionUpper.'_DIRECTORY', method_exists($this->SP_WBC_Extension, 'DIRECTORY') ? $this->SP_WBC_Extension->DIRECTORY() : $plugin_dir_path );
			
			// defined('EOWBC_BASE_DIRECTORY') || define('EOWBC_BASE_DIRECTORY', basename(__DIR__));
			defined( $singleton_functionUpper.'_BASE_DIRECTORY') || define($singleton_functionUpper.'_BASE_DIRECTORY', method_exists($this->SP_WBC_Extension, 'BASE_DIRECTORY') ? $this->SP_WBC_Extension->BASE_DIRECTORY() : basename($plugin_dir_path) );
			
			// defined('EOWBC_NAME') || define('EOWBC_NAME', 'Woo Choice Plugin' /*$plugin_data['Name']*/);
			defined( $singleton_functionUpper.'_NAME') || define($singleton_functionUpper.'_NAME', method_exists($this->SP_WBC_Extension, 'NAME') ? $this->SP_WBC_Extension->NAME() : $plugin_data['Name'] );
			...
			defined( $singleton_functionUpper.'_HELPERS_DIR') || define($singleton_functionUpper.'_HELPERS_DIR', method_exists($this->SP_WBC_Extension, 'HELPERS_DIR') ? $this->SP_WBC_Extension->HELPERS_DIR() : constant($singleton_functionUpper.'_DIRECTORY').'application/helper/' );
			...
			defined( $singleton_functionUpper.'_TEMPLATE_DIR') || define($singleton_functionUpper.'_TEMPLATE_DIR', method_exists($this->SP_WBC_Extension, 'TEMPLATE_DIR') ? $this->SP_WBC_Extension->TEMPLATE_DIR() : constant($singleton_functionUpper.'_DIRECTORY').'application/view/' );

			//new _TEMPLATE_DIR
			defined( $singleton_functionUpper.'_TEMPLATE_DIR_EXTENDED') || define($singleton_functionUpper.'_TEMPLATE_DIR_EXTENDED', method_exists($this->SP_WBC_Extension, 'TEMPLATE_DIR_EXTENDED') ? $this->SP_WBC_Extension->TEMPLATE_DIR() : constant($singleton_functionUpper.'_DIRECTORY').'templates/' );
			...
			defined( $singleton_functionUpper.'_ICON') || define($singleton_functionUpper.'_ICON', method_exists($this->SP_WBC_Extension, 'ICON') ? $this->SP_WBC_Extension->ICON() : constant($singleton_functionUpper.'_ASSET_DIR').'icon/mini.png' );
		...
		public function theme_adaption_check() {
			...
			if( is_admin() ) {
				...
				if( strpos($page_slug, "---theme-adaption") !== FALSE ) {
					...
					if( $curr_plugin_slug == $this->SP_WBC_Extension->extension_slug() ) {

						add_filter('sp_wbc_theme_adaption_config', function( $plugin_slug ) {

							if( $plugin_slug == $this->SP_WBC_Extension->extension_slug() ) {
								return $this->SP_WBC_Extension->singleton_function()()->config->required_hooks_n_filters_etc();
							}
						}, 10, 1);
					}
			...
			if( !empty(wbc()->sanitize->get('thadc')) && wbc()->sanitize->get('thadc') == 1 ) {
				add_action('sp_wbc_theme_adaption_check',function(){
					...
					\eo\wbc\model\utilities\SP_WBC_Theme_Adaption_Check::instance()->check( $this->SP_WBC_Extension->singleton_function()()->config->required_hooks_n_filters_etc() );
				},20);
			}
		...
		public function __init( $childClassObj ) {
			...
			if( wbc()->sanitize->get('is_test') == 1 ) {
				...
				wbc_pr("SP_WBC_Plugin_Index_Class __init");
			}
			...
			// add_action( 'after_eowbc_load', function() use($childClassObj) {
				...
				do_action( 'before_spext_init', $this->SP_WBC_Extension->extension_slug() );
				...
				(new SP_WBC_Extensions_Bootstrap( $this->SP_WBC_Extension ))->run();

				do_action( 'after_spext_init', $this->SP_WBC_Extension->extension_slug() );
		...
		public function plugin_load_process( SP_WBC_Extension $SP_Extension ) {
			...
			add_filter('plugin_action_links_'. plugin_basename( $this->getFILE() ),function($links){
			    ...
			    $links[] = '<a href="' .
			        esc_url(admin_url( 'admin.php?page='.$this->SP_WBC_Extension->admin_page_slug() )) .
			        '">' . esc_html__('Admin Panel',$this->SP_WBC_Extension->extension_slug())  . '</a>';
			        return $links;
			...
			if( false ) {
				...
				register_activation_hook( $this->getFILE(), 'SP_WBC_Extensions_Bootstrap::activate');
				register_deactivation_hook( $this->getFILE(), 'SP_WBC_Extensions_Bootstrap::deactivate');
				register_uninstall_hook( $this->getFILE(), 'SP_WBC_Extensions_Bootstrap::uninstall');
			}
