...
class SP_WBC_Configuration {
	...
	public function save( $form_definition ) {
		...
		if(!empty(wbc()->sanitize->post('first_name'))) {
			wbc()->options->update_option('configuration','first_slug',@\eo\wbc\model\SP_WBC_Category_Attribute::instance()->get_single_category((int)wbc()->sanitize->post('first_name'))->slug );
		}

		if(!empty(wbc()->sanitize->post('second_name'))) {
			wbc()->options->update_option('configuration','second_slug',@\eo\wbc\model\SP_WBC_Category_Attribute::instance()->get_single_category(wbc()->sanitize->post('second_name'))->slug);
		}
