			...
			foreach($filters as $filter){        						
        		...
        		// if( !in_array($filter['name'], $names)){
                    ...
                    if(empty($tprefix)){
    					...
    					$res = \eo\wbc\model\admin\SP_WBC_Filters::instance()->save( \eo\wbc\controllers\admin\menu\page\Filters::get_form_definition(), true );
                    } else {
                        if( $tprefix == "sc_" ) {
                            $res = \eo\wbc\model\admin\SP_WBC_Shop_Category_Filter::instance()->save( \eo\wbc\controllers\admin\menu\page\Shop_Category_Filter::get_form_definition(), true );
                        }
                        elseif( $tprefix == "shortflt_" ) {
                            $res = \eo\wbc\model\admin\SP_WBC_Shortcode_Filters::instance()->save( \eo\wbc\controllers\admin\menu\page\Shortcode_Filters::get_form_definition(), true );
                        } elseif( $tprefix == "eorad_sc_" ) {
                            $res = \eo\rad\model\admin\Eorad_Filter::instance()->save( \eo\rad\controllers\admin\pages\Request_Diamond_Filter::get_form_definition(), true );
                        }
                        else {
	...
	public function fc3() {
        ...
        if(!empty($__cat__['eo_lab_diamond_shape_cat'])){
            $this->filter['d_fconfig'][]=array(
                ...
                'template'=>'fc3',
                'help'=>1,
                'help_text'=>'Shape of the lab Grown',
                'enabled'=>1
            );
        }
        if(!empty($__att__['eo_carat_attr'])){
	...
	public function fc4() {
        ...
        if(!empty($__cat__['eo_lab_diamond_shape_cat'])){
            $this->filter['d_fconfig'][]=array(
                'name'=>$__cat__['eo_lab_diamond_shape_cat'][0],
                'type'=>"0",
                'label'=>"Lab Grown",    //$__cat__['eo_lab_diamond_shape_cat'][1],
                'advance'=>"0",
                'dependent'=>"0",
                'input'=>"icon_text",
                'column_width'=> "50",
