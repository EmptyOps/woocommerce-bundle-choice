...
class Form_Builder implements Builder {
		...
		if( !in_array("backcolor", $hide_defaults) )
		{
			...
			for($fi=0; $fi<$ftot; $fi++)
			{
				...
				$collection[$field_id.'_backcolor_lbl'.$field_id_suffix] = array(
					...
					/* Language function - comment */ 
					'info_icon'=>array(
						'text'=>( array_key_exists("backcolor", $info_text_overrides) ? $info_text_overrides["backcolor"] : __('Sets specified color as background color on '.$field_label.( $special_lbl == "" ? "" : ' while its '.$special_lbl),'woo-bundle-choice') ), 'type'=>'info_icon', ),
				);
	...
	public static function savable_types() {
		return array("text","checkbox","color","hidden","radio","select","textarea","icon","time");
	}
