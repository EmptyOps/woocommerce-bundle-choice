...
if(wp_verify_nonce(wbc()->sanitize->post('_wpnonce'),'sp_wbc_mapping')) {                

	wbc()->load->model('admin/sp_wbc_mapping');
	wbc()->load->model('admin\form-builder');
	    
	if( isset($_POST["sub_action"]) && wbc()->sanitize->post("sub_action") == "bulk_delete" ) {
		$res = eo\wbc\model\admin\SP_WBC_Mapping::instance()->delete( wbc()->sanitize->post_array("ids"), wbc()->sanitize->post("saved_tab_key") );
	} elseif(isset($_POST['sub_action']) and wbc()->sanitize->post('sub_action') == 'fetch') {
		$res = eo\wbc\model\admin\SP_WBC_Mapping::instance()->fetch_map($res);
	} elseif (isset($_POST['sub_action']) and wbc()->sanitize->post('sub_action') == 'fetch_product') {
		
		$res = \eo\wbc\model\SP_WBC_Search_Product::instance()->get_ajax();
