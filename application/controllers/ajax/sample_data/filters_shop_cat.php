...
try {
	...
	if(wp_verify_nonce(wbc()->sanitize->post('_wpnonce'),'sample_data_jewelry')){                
		...
		// else {
			...
			$res /*echo*/ = \eo\wbc\model\admin\sample_data\Eowbc_Filters_Shop_Cat::instance()->create_product(intval(wbc()->sanitize->post('product_index')));
			// wp_die();
	    // }
    ...
    $res = array(
        "type" => "error",
        "msg"  => !empty($errorMessage) ? $errorMessage : "There is some error in this sample data PHP process for the feature 'Shop/Category Filters'."
    );
    ...
    $res = array(
        "type" => "error",
        "msg"  => $e->getMessage() ?: "There is some error in this sample data PHP process for the feature 'Shop/Category Filters'."
    );
