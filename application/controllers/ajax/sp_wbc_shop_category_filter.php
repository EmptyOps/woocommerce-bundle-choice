...
if(wp_verify_nonce(wbc()->sanitize->post('_wpnonce'),'sp_wbc_shop_category_filter')) {                
	
	wbc()->load->model('admin/sp_wbc_shop_category_filter');
	wbc()->load->model('admin\form-builder');
	    
	if( isset($_POST["sub_action"]) && wbc()->sanitize->post("sub_action") == "bulk_delete" ) {
		$res = eo\wbc\model\admin\SP_WBC_Shop_Category_Filter::instance()->delete( wbc()->sanitize->post_array("ids"), wbc()->sanitize->post("saved_tab_key"),0 );
	} elseif( isset($_POST["sub_action"]) && wbc()->sanitize->post("sub_action") == "bulk_activate" ) {
		$res = eo\wbc\model\admin\SP_WBC_Shop_Category_Filter::instance()->activate( wbc()->sanitize->post_array("ids"), wbc()->sanitize->post("saved_tab_key"),1 );
	} elseif( isset($_POST["sub_action"]) && wbc()->sanitize->post("sub_action") == "bulk_deactivate" ) {
		$res = eo\wbc\model\admin\SP_WBC_Shop_Category_Filter::instance()->deactivate( wbc()->sanitize->post_array("ids"), wbc()->sanitize->post("saved_tab_key"),1 );
	} elseif(isset($_POST['sub_action']) and wbc()->sanitize->post('sub_action') == 'fetch') {
		$res = eo\wbc\model\admin\SP_WBC_Shop_Category_Filter::instance()->fetch_filter($res);
	}
	else {
		$res = eo\wbc\model\admin\SP_WBC_Shop_Category_Filter::instance()->save( eo\wbc\controllers\admin\menu\page\Shop_Category_Filter::get_form_definition() );
    }
