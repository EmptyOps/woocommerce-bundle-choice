...
class Controller extends \eo\wbc\controllers\Controller{
    ...
    protected function get_ui_definition($args = array()){
       ...
       // ACTIVE_TODO move to asana
        ...
        -> @a ACTIVE_TODO_OC_START - END remove karvu
        if(!empty($data['sp_localize_key']) && !empty($data['sp_localize_data']){
            ...
            wbc()->load->asset("sp_localize", "", array(), "", true, false, $data['sp_localize_key'], $data['sp_localize_data']);
            ...
            unset($data['sp_localize_key']);
            unset($data['sp_localize_data']);
        }
        ACTIVE_TODO_OC_END*/
