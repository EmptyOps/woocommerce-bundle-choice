...
if (!class_exists('Extensions_Admin_Menu')) {
	class Extensions_Admin_Menu extends Admin_Menu
	{
		...
		public function get_menu()
		{
			...
			if (wbc()->sanitize->get('page') == $this->SP_Extension->admin_page_slug() || (wbc()->sanitize->get('page') == $this->SP_Extension->extension_slug() . '---theme-adaption' || wbc()->sanitize->get('page') == $this->SP_Extension->extension_slug() . '---sp-queue' || wbc()->sanitize->get('page') == $this->SP_Extension->extension_slug() . '---extras')) {
				...
				//config array should be defined, if not then do not add submenu
				if (isset($this->SP_Extension->singleton_function()()->config) && method_exists($this->SP_Extension->singleton_function()()->config, 'extras')) {
		...
		public function define_extras_menu()
		{

			$theme_adaption_menu = parent::define_extras_menu();
			$theme_adaption_menu['parent_slug'] = 'eowbc';	// $this->SP_Extension->admin_page_slug();
			$theme_adaption_menu['title'] = eowbc_lang('Extras') . ' - ' . $this->SP_Extension->extension_name() . ' - ' . constant('EOWBC_NAME');
			$theme_adaption_menu['slug'] = $this->SP_Extension->extension_slug() . '---extras';
