...
class WBC_WC {
    ...
    public static function eo_wbc_create_attribute($args){
        ...
        if(function_exists('wc_create_attribute')) {
            ...
            if ( strlen( $slug ) >= 28 ) {
                return new WP_Error( 'invalid_product_attribute_slug_too_long', sprintf( __( eowbc_lang('Name "%s" is too long (28 characters max). Shorten it, please.'), 'woocommerce' ), $slug ), array( 'status' => 400 ) );
            } elseif ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
                return new WP_Error( 'invalid_product_attribute_slug_reserved_name', sprintf( __( eowbc_lang('Name "%s" is not allowed because it is a reserved term. Change it, please.'), 'woocommerce' ), $slug ), array( 'status' => 400 ) );
            } elseif ( taxonomy_exists( wc_attribute_taxonomy_name( $args['name'] ) ) ) {
                return new WP_Error( 'invalid_product_attribute_slug_already_exists', sprintf( __( eowbc_lang('Name "%s" is already in use. Change it, please.'), 'woocommerce' ), $args['name'] ), array( 'status' => 400 ) );
            }
