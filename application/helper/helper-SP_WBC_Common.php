...
class SP_WBC_Common {
    ...
    public function sp_wbc_current_theme_key() {
	...
	public function is_mobile_by_page_sections($key = null, $is_other_theme = false) {
		...
		if (wp_is_mobile()) {
			...
			$theme_key = wbc()->common->sp_wbc_current_theme_key();
...
function sp_wbc_var_dump($v, $force_debug = false,$die = false, $add_br = true) {
		
	return wbc()->common->var_dump($v, $force_debug, $die, $add_br);

}

function sp_wbc_free_memory( &$var ) {
		
	return wbc()->common->free_memory($var);

}

function sp_wbc_consistsOfTheSameValues(array $a, array $b, bool $strict = false) {
	   
   	return wbc()->common->consistsOfTheSameValues($a, $b, $strict);
}

function sp_wbc_nonZeroEmpty(&$var) {

	return wbc()->common->nonZeroEmpty($var);
}

function sp_wbc_createUniqueId() {

	return wbc()->common->createUniqueId();

}

function sp_wbc_stringToKey( $str )
{
...
function sp_wbc_createUniqueHashId(array $a, array $fields_to_use, string $prefix = "") {

	return wbc()->common->createUniqueHashId($a, $fields_to_use, $prefix);

}

function sp_wbc_array_column($input = null, $columnKey = null, $indexKey = null) { 
    
	return wbc()->common->array_column($input, $columnKey, $indexKey);
    
}

function sp_wbc_http_query($param){

	return wbc()->common->http_query($param);

}

function sp_wbc_is_ajax(){

	return wbc()->common->is_ajax();

}

function sp_wbc_is_object($obj){
...
function sp_wbc_current_theme_key() {

	return wbc()->common->sp_wbc_current_theme_key();

}

function sp_wbc_site_url($slug='', $query_string=''){

	return wbc()->common->site_url($slug, $query_string);

}

function sp_wbc_makeNestedArray($keys, $value, $target_array=null)
{
...
function sp_wbc_arrValuesToNestedArr($arr, $target_value, $target_array=null) { 

	return wbc()->common->arrValuesToNestedArr($arr, $target_value, $target_array);

}

function sp_wbc_special_characters() {

	return wbc()->common->special_characters();

}

function sp_wbc_key_to_title( $key, $is_basic_conversion = false ) {

	return wbc()->common->key_to_title($key, $is_basic_conversion);

}

function sp_wbc_truncate($str,$limit) {
...
function sp_wbc_placeholder_img_src() {
