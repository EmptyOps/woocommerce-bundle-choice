...
edit_codeception_yml_file() {
	...
	if [[ $(test_environment) == "WBC_TEST_ENV_default" ]]; then
	  ...
	  sed $ioption "s|http://127.0.0.1:8888|http://127.0.0.1:8888/wordpress/src|" "$TRAVIS_BUILD_DIR"/tests/acceptance.suite.yml
	  sed $ioption "s|http://127.0.0.1:8888|http://127.0.0.1:8888/wordpress/src|" "$TRAVIS_BUILD_DIR"/codeception.dist.yml
	else 
