...
class YC_Orders
{
   public function __construct()
   {
       add_action('woocommerce_admin_order_data_after_order_details',function($order){
           global $wpdb;
           $sets=$wpdb->get_row('select * from `'.$wpdb->prefix.'yc_order_maps` where order_id='.$order->id,'ARRAY_A');
           $sets=(json_decode($sets['order_map']));
           add_action('admin_footer',function() use ($sets){
                 echo "<script>
                        if(document.getElementById('order_items_list')){
                            document.getElementById('order_items_list').innerHTML='".call_user_func_array(array(__CLASS__,'get_sets'),[$sets])."';
                        }
                        else
                        {
                            document.getElementById('order_line_items').innerHTML='".call_user_func_array(array(__CLASS__,'get_sets'),[$sets])."';
                        }                        
                    </script>";
           });
       });
   ...
   {   
       $price=0;
       $row="<tr>".
           "<td style=\"min-width:330px;vertical-align: middle;\">".YC_Support::wc_get_product($set[0][0])->get_image("thumbnail")."&nbsp;";
       if($set[1]){
           $row.=YC_Support::wc_get_product($set[1][0])->get_image("thumbnail");
       }
       $row.="</td>".
           "<td style=\"vertical-align: middle;\">".
           "<h5>".YC_Support::wc_get_product($set[0][0])->get_title().($set[0][2]  ? "<br/>&nbsp; -".implode(',',YC_Support::wc_get_product_variation_attributes($set[0][2])):'')."</h5>";
       if($set[1]){
           $row.="<h5>".YC_Support::wc_get_product($set[1][0])->get_title().($set[1][2]  ? "<br/>&nbsp; -".implode(',',YC_Support::wc_get_product_variation_attributes($set[1][2])):'')."</h5>";
       }
       $row.="</td>".
           "<td style=\"vertical-align: middle;\">".
