    ...
    if(isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'],'eo_wbc_auto_jewel')) {
      ...
      while(!empty($_POST['attr_value_'.$index])){
        if(!empty($_POST['attr_'.$index])){
          $_atttriutes[$index]['name']=$_POST['cat_value_'.$index];
      ...
      $catat=new EO_WBC_CatAt();
          ...
          if(!empty($category)){
            ...
            update_option(serialize($catat_category));
          }

          if(!empty($attributes)){
            //Send for creation and update returned array.
            $catat_attribute=$catat->create_attribute($attributes);            
            update_option(serialize($catat_attribute));
          } 
                ...
                array(
                  ...
                  'variation'=>array(   
                                  ...
                                  array(
                                    'regular_price'=>'10',
                                    'price'=>'7',                                    
                                    'terms'=>array('pa_colour'=>'f','pa_fluorescence'=>'faint')
                                  )
                                )  
                    )    
              ));

        header("Location: ".admin_url('admin.php?page=eo-wbc-home')); 
        exit();
      }
      else{
        $_step=$_POST['step'];
        ...
        <table class="form-table">
          <tbody>
            <tr valign="top">
              ...
              <tr>
                <td> A sample products of diamond and rings.</td>
          ...
          <tfoot>
            <tr>
              <td>
                ...
                <input type="submit" name="save" value="<?php printf(__("Create these %1s","woo-bundle-choice"),$_steps[$_step-1]); ?>"  class="button button-primary button-hero action">                
              </td>
              <td>
                <a href="#" class="button button-hero action" onclick="window.location.href='<?php echo admin_url('admin.php?page=eo-wbc-home'); ?>'">Cancel</a>
              </td>
            </tr>
          </tfoot>
        </table>
