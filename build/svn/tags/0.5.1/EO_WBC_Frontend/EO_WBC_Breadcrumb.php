...
class EO_WBC_Breadcrumb
{
    ...
    public static function eo_wbc_add_breadcrumb($step=1,$begin){
        ...
        //url for change button and product view button as these also matters when navigating through the steps.
        $html.='><a href="'.( (empty($_GET['EO_CHANGE']) XOR empty($_GET['EO_VIEW']) ) && !empty($_GET['FIRST']) && !empty($_GET['SECOND'])?get_bloginfo('url')
                    ...
                    .'&STEP=3&FIRST='.sanitize_text_field($_GET['FIRST']).'&SECOND='.sanitize_text_field($_GET['SECOND']):'#' ).'"><div><span class="step">'.
                (
                    get_option('eo_wbc_show_hide_breadcrumb_icon','0')?(
                    get_option('eo_wbc_collection_icon')
                        ?
                        "<img src='".wp_get_attachment_url(get_option('eo_wbc_collection_icon'))."'/>"
                        :
                        '3'):''
                ).'</span><span class="step-name">'.(get_option('eo_wbc_collection_title')?get_option('eo_wbc_collection_title'):"Preview").'</span><div></a></li></div>';        
        return $html;             
    }
    ...
    private static function eo_wbc_breadcumb_first_html($step,$order){
        ...
        $html.='><a href="'.
        (empty($_GET['FIRST'])?'#':self::eo_wbc_breadcrumb_view_url(sanitize_text_field($_GET['FIRST']),$order)).'"><div><span class="step">'.
                (
                    get_option('eo_wbc_show_hide_breadcrumb_icon','0')?(
                    get_option('eo_wbc_first_icon')
                        ?
                        "<img src='".wp_get_attachment_url(get_option('eo_wbc_first_icon'))."'/>"
                        :
                        $order):''
                ).' </span><span class="step-name">'.get_option('eo_wbc_first_name').'</span></div></a><div>
    ...
    private static function eo_wbc_breadcumb_second_html($step,$order){
        ...
        $html.='><a href="'.
        (empty($_GET['SECOND'])?'#':
        self::eo_wbc_breadcrumb_view_url(sanitize_text_field($_GET['SECOND']),$order)).'"><div><span class="step">'.
                (
                    get_option('eo_wbc_show_hide_breadcrumb_icon','0')?(
                    get_option('eo_wbc_second_icon')
                        ?
                        "<img src='".wp_get_attachment_url(get_option('eo_wbc_second_icon'))."'/>"
                        :
                        $order):''
                ).' </span><span class="step-name">'.get_option('eo_wbc_second_name').'</span></div></a><div>
