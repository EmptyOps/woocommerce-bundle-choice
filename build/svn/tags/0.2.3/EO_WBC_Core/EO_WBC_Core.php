...
class EO_WBC_Core{
    ...
    public static function eo_wbc_activate(){
        ...
        if($wpdb->get_var( "show tables like '$eo_wbc_cat_map'" ) != $eo_wbc_cat_map)
        {
            ...
            $sql .= " `first_cat_id` VARCHAR(125) NOT NULL ,";
	    $sql .= "`second_cat_id` VARCHAR(125) NOT NULL ,";
	    $sql .= "`discount` VARCHAR(20) not null DEFAULT '0%' ,";
	    $sql.=" PRIMARY KEY (`first_cat_id`, `second_cat_id`)";
            $sql .= ") ".$wpdb->get_charset_collate().";";                        
            dbDelta($sql);            
        }
