	...
	class EO_WBC_First_Filter_Table extends WP_List_Table{
		...
		public function get_filters(){
        	...
        	if(count($filters_data)>0){
            	...
            	foreach ($filters_data as $key=>$item) {
                    ...
                    if(!empty(wc_get_attribute($item['name']))){
                        $item_name=wc_get_attribute($item['name'])->name;    
                    }
                    else{
		...
		function column_cb($item) {
			...
			$actions = array(
		        ...
		        'delete'    => sprintf('<a href="?page=%s&action=%s&name=%s&eo_wbc_action=%s&eo_filter_target=%s">Delete</a>',$_REQUEST['page'],'delete',$item['id'],'single_filter_delete','eo_wbc_add_filter_first'),
		    );
