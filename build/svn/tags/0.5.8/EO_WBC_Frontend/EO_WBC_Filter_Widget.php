...
class EO_WBC_Filter_Widget {
	...
	private function eo_wbc_filter_enque_asset()
	{
        ...
        wp_localize_script('eo_wbc_filter_js','eo_wbc_object',array(
        					...
        					'disp_regular'=>!get_option('eo_wbc_e_tabview_status',false),
	...
	private function get_widget() {
		...
		foreach ($filter as $key => $item) {
			...
			if($item['advance']==0){
				$item['order']=empty($item['order'])?(-1*count($non_adv_ordered_filter)):empty($item['order']);
				$non_adv_ordered_filter[$item['order']]=$item;
			}
			else{
				$item['order']=empty($item['order'])?(-1*count($adv_ordered_filter)):empty($item['order']);
		...
		foreach ($non_adv_ordered_filter as $key => $item) {
			...
			elseif($item['type']==1 ) {
				...
				$_attr_list[]=wc_get_attribute($item['name'])->slug;
			}
		}
		...
		if( !empty($adv_ordered_filter) ){
			...
			echo "<div class='eo_wbc_advance_filter' style='margin-bottom:1.5em;'><h1 style='text-align:center'><a href='#'>".__('Advance Filter',"woo-bundle-choice")."</a></h1><div class='eo_wbc_advance_filter_display'>";
		...
		echo '<input type="hidden" name="_attribute" id="eo_wbc_attr_query" value="'.implode(',',$_attr_list).'" />';		
		echo "</div></div></form>";
		/*foreach ($filter as $key => $item) {			
