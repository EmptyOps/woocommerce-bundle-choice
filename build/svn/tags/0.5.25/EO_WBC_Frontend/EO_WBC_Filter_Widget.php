...
class EO_WBC_Filter_Widget {
	...
	private function get_widget() {
		...
		<div id="loading"></div>
		<div class="ui grid mobile only container" style="margin-left: 0; margin-right: 0">
			<div class="row">
	...
	foreach ($adv_ordered_filter as $key => $item) {
  	...
  	foreach ($adv_ordered_filter as $key => $item) {
		...
		<div class="ui grid centered">
			<div class="row">
				<div class="ui button primary" id="advance_filter" style="border-radius: 0 0 0 0;">Advance Filter&nbsp;<i class="ui icon angle double down"></i></div>
			</div>
		</div>			
		<?php endif; ?>
		...
		<script type="text/javascript">		
			...
			jQuery(document).ready(function($){			
				...
				if($(primary_computer_only).css('display')!='none'){				
					
					window.eo.slider($(primary_computer_only).find(".slider"));

				}else if($(primary_mobile_only).css('display')!='none'){
					
					primary_mobile_accordion=$(primary_mobile_only).find('.ui.styled.fluid.accordion');

					$(primary_mobile_accordion).accordion();				

					$(primary_mobile_accordion).find('.title').each(function(i){
						$(primary_mobile_accordion).accordion('open',i);
						window.eo.slider($(this).next().find('.ui.slider'));					
					});			

					$(primary_mobile_accordion).accordion("close",$(primary_mobile_accordion).find('.title').length-1);
				}
				...
				if($(secondary_computer_only).css('display')!='none'){				
					...
					window.eo.slider($(secondary_computer_only).find(".slider"));
					...
					secondary_mobile_accordion=$(secondary_mobile_only).find('.ui.styled.fluid.accordion');

					$(secondary_mobile_only).accordion();				

					$(secondary_mobile_only).find('.title').each(function(i){
						$(secondary_mobile_only).accordion('open',i);
						window.eo.slider($(this).next().find('.ui.slider'));					
					});			

					$(secondary_mobile_only).accordion("close",$(secondary_mobile_only).find('.title').length-1);
				...
				$('.checkbox').checkbox({onChange:function(){
					...
					console.log($(this).attr('data-filter-slug'));
