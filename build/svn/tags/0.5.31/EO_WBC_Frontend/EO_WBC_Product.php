...
class EO_WBC_Product {
    ...
    */
    ...
    private function eo_wbc_get_category()
    {
        ...
        {
            if(!empty($_GET['BEGIN']) && $_GET['BEGIN']==$first_cat && !empty($_GET['STEP']) && $_GET['STEP']==1){
                return $first_cat;
            }
            elseif(!empty($_GET['BEGIN']) && $_GET['BEGIN']==$first_cat && !empty($_GET['STEP']) && $_GET['STEP']==2){
                return $second_cat;
            }
        }
        elseif(count(array_intersect($terms,$second_cat_list))>0)
        {
            if(!empty($_GET['BEGIN']) && $_GET['BEGIN']==$second_cat && !empty($_GET['STEP']) && $_GET['STEP']==1){
                return $second_cat;
            }
            elseif(!empty($_GET['BEGIN']) && $_GET['BEGIN']==$second_cat && !empty($_GET['STEP']) && $_GET['STEP']==2){
                return $first_cat;
            }
        }
    } 
