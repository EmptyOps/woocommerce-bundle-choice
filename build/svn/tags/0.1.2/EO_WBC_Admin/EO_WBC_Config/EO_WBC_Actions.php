...
class EO_WBC_Actions
{
    ...
    {                  
            ...
            elseif(
                ...
                && !empty($_POST['eo_wbc_btn_page'])
                && !empty($_POST['eo_wbc_btn_position'])                
                && !empty($_POST['eo_wbc_btn_custome_after'])
                ){
                    $this->config_save();
            }            
    }
    ...
    {          
        ...
        if ($_POST['eo_wbc_btn_setting']=="0") { //if default is choosen
            ...
            //position of button (Top/Bottom/Middle/Custome)
            update_option('eo_wbc_btn_position',$_POST['eo_wbc_btn_position']);
            if($_POST['eo_wbc_btn_position']=='custome')//If custome position
            {
                //Custome position to show buttons
                update_option('eo_wbc_btn_custome_after',$_POST['eo_wbc_btn_custome_after']);
            }
        }
        else
        {
            /**
            * Save page slug which is required to get back when 'continue shopping' is triggred.
            */
            update_option('eo_wbc_btn_page',$_POST['eo_wbc_btn_page']);
        }
