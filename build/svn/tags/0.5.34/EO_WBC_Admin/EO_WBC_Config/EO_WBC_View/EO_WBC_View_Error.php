...
<h1></h1>
  ...
  <br/>
	...
	<label for="agree_terms" style="position: absolute; right:1em;"><input type="checkbox" name="agree_terms" id="agree_terms" onchange="if(jQuery(this)[0].checked){ jQuery('#send_report').removeClass('disabled'); } else { jQuery('#send_report').addClass('disabled'); }"> I agree with sphereplugin's <a href="https://sphereplugins.com/terms-conditions/">Terms & Policy</a>.</label>
	<br/>
	<br/>
	<div style="float:right;">
		<input type="submit" value="Cancel" class="submit button" name="clear" onclick="window.location.href=document.referrer">
		<input type="submit" value="Send error report" class="submit button-primary disabled" name="report" id="send_report" onclick="if(jQuery('#agree_terms')[0].checked){ window.location.href='<?php echo admin_url('admin.php?page=eo-wbc-logs&eo_wbc_view_error=1&action=report'); ?>'} else { return false }">	
		<br/><br/>
		<a href="<?php echo admin_url('admin.php?page=eo-wbc-logs&eo_wbc_view_error=1&action=clear&ref='.
		(empty($_SERVER['HTTP_REFERER'])? admin_url('admin.php?page=eo-wbc-logs'):$_SERVER['HTTP_REFERER'])); ?>"><?php _e("Clear log and retrun.","woo-bundle-choice"); ?></a>
	</div>
