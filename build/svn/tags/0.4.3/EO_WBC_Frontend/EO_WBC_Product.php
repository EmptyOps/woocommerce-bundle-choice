...
class EO_WBC_Product
{
    ...
    private function eo_wbc_make_pair(){
        ...
        if(
            ...
            add_action('woocommerce_after_add_to_cart_button',function(){                
                echo "<a href='#' id='eo_wbc_add_to_cart' class='single_add_to_cart_button button alt make_pair'>".get_option('eo_wbc_pair_text')."</a>";
            });
            //Add css to the head
    ...
    {        
        ...
        add_filter('woocommerce_product_single_add_to_cart_text',function(){
            return __(get_option('eo_wbc_add_to_cart_text','Continue'),'woocommerce');
        });
    }
       	...
       	<script type="text/javascript">
    		jQuery(".single_add_to_cart_button.button.alt").ready(function(){
    			...
    			jQuery(".single_add_to_cart_button.button.alt:not(.disabled)").replaceWith(
                     ...
                     +"<?php _e(get_option('eo_wbc_add_to_cart_text','Continue')); ?>"
                     +"</a>"
                    );
    			});
    	</script>
    ...
    private function eo_wbc_category_link($variable_status=FALSE){
        ...
        if(is_array($cat) && count($cat)>0){
            foreach ($cat as $term_id){
                $glue=get_option('eo_wbc_map_cat_pref','and')==='and'?'+':',';
                $link.=get_term_by('term_taxonomy_id',$term_id,'category')->slug.$glue;    
            }                        
        ...
        if(is_array($tax) && count($tax)>0){            
            ...
            foreach ($filter_query as $filter_name => $filters) { 
                $glue=get_option('eo_wbc_map_attr_pref','or')==='or'?',':'+';           
                $link.="query_type_{$filter_name}=or&filter_{$filter_name}=".implode($glue,$filters)."&";
            }       
        }        
        return $link;
    }
