...
?>
	<div class="<?php echo esc_attr($width_class); ?>" data-tab-group="<?php esc_attr_e($tab_set); ?>">
	    ...
	    <div class="ui tiny images ui equal width center aligned grid" style="text-align: center;" data-reset="reset_icon(new Event('click'),'<?php echo esc_attr($term->slug); ?>')" data-filter-slug="<?php echo esc_attr($term->slug); ?>">
	        <?php foreach ($list as $filter_icon) : ?>
	            <div title="<?php esc_attr($filter_icon["name"]); ?>"
				    ...
				    <div>
				        <img src='<?php echo esc_url($filter_icon['mark'] ? $filter_icon['select_icon'] : $filter_icon['icon']); ?>' 
				             data-imgsrc="<?php echo esc_url($filter_icon['icon']); ?>" 
				             data-toggleimgsrc="<?php echo esc_url($filter_icon['select_icon']); ?>"/>
				    </div>
