...
if(!empty($id) /*and !empty($label)*/){
	...
	<div class="<?php echo !empty($size_class) ? esc_attr($size_class) : ''; ?> field">
		...
		?>	
		<input type="color" <?php /*phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- we are not escaping the $attr because it may contain multiple attributes and we do not want esc_attr function to escape the double qoutes(") of the different attribute values. but anyway we understand the security requirement here so we have always escaped the attribute value with the esc_attr from wherever this $attr var is passed*/echo !empty($attr) ? $attr : ''; ?> id="<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($id); ?>" class="<?php echo !empty($class) ? esc_attr($class) : ''; ?>" placeholder="<?php echo !empty($placeholder) ? esc_attr($placeholder) : ''; ?>" value="<?php echo !empty($value) ? esc_attr($value) : ''; ?>">
		<?php
